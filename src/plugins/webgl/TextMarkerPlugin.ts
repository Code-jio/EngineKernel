...
interface TextMarkerConfig {
    ...
    image?: ImageConfig
    scale?: number
    fixedScale: boolean // 如果为true，根据标记与相机之间的距离动态调整大小；如果为false，使用固定的缩放比例
    rotation?: number
    visible?: boolean
    autoSize?: boolean
    minSize?: number
...
export class TextMarkerPlugin extends BasePlugin {
    ...
    public markerInstances: Map<string, MarkerInstance> = new Map() // 创建的sprite列表
    public instanceIdCounter: number = 0
    private raycaster: THREE.Raycaster = new THREE.Raycaster()
    private mouse: THREE.Vector2 = new THREE.Vector2()
    public enableDebugMode: boolean = false
    public defaultTextStyle: TextStyle
    ...
    private drawBackground(
        ...
        if (backgroundStyle.gradient) {
            ...
            backgroundStyle.gradient.colors.forEach((color, index) => {
                const stop =
                    backgroundStyle.gradient!.stops?.[index] ?? index / (backgroundStyle.gradient!.colors.length - 1)
    ...
    private updateSpriteSize(instance: MarkerInstance, size: { width: number; height: number }): void {
        if (instance.config.scale) {
            if (instance.config.fixedScale) {
                instance.sprite.scale.set(
                    instance.config.scale * size.width * 0.03,
                    instance.config.scale * size.height * 0.03,
                    1,
                )
            } else {
                instance.sprite.scale.set(
                    instance.config.scale * size.width * 0.0005,
                    instance.config.scale * size.height * 0.0005,
                    instance.config.scale * 0.0005,
                )
            }
        }
    }
    ...
    public updateMarker(markerId: string, config: Partial<TextMarkerConfig>): boolean {
        ...
        if (config.visible !== undefined) {
            instance.sprite.visible = config.visible
            instance.isVisible = config.visible
        }
    ...
    public addMarkerBatch(configs: TextMarkerConfig[]): Promise<string[]> {
        return new Promise(resolve => {
            ...
            // 等待所有标记渲染完成
            Promise.all(markerIds.map(id => this.renderMarker(id))).then(() => {
