...
export class Raycast extends BasePlugin {
    ...
    getSomeThingByRay(event: MouseEvent) {
        ...
        (this.raycaster as THREE.Raycaster).setFromCamera(
            this.mouse as THREE.Vector2,
            this.camera as THREE.PerspectiveCamera,
        );

        const intersects = (this.raycaster as THREE.Raycaster).intersectObjects(
            (this.scene as THREE.Scene).children,
            true,
        );
