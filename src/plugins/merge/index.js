...
export class StaticGeometryMerger extends BasePlugin {
    ...
    _findStaticMeshes() {
        ...
        this.scene.traverse((object) => {
            ...
            // 跳过动态对象
            if (this._isDynamicObject(object, dynamicKeywords)) return
            
            // 跳过特殊对象
            if (object.userData.noMerge || object.userData.dynamic) return
            
            // 跳过透明对象（避免排序问题）
            if (object.material && object.material.transparent) return
            
            // 跳过LOD对象（保留层级关系）
            if (this.mergeConfig.preserveLOD && object.parent && object.parent.isLOD) return
     ...
     * 判断是否为动态对象
     */
    _isDynamicObject(object, dynamicKeywords) {
     ...
     * 按材质分组合并
     */
    _groupMeshesByMaterial(objects) {
     ...
     * 获取单个材质键
     */
    _getSingleMaterialKey(material) {
        const props = [
            material.type,
    ...
    _getWorldGeometry(object) {
        ...
        // 应用世界变换
        if (!object.matrixWorld.isIdentity()) {
     ...
     * 创建合并后的网格
     */
    _createMergedMeshes(materialGroups) {
        const mergedMeshes = []
    ...
    _buildBVHAcceleration() {
        this.mergedGroups.forEach((group, materialKey) => {
            ...
            try {
                ...
                group.mesh.geometry.boundsTree = bvh
                
                console.log(`[StaticGeometryMerger] BVH构建完成: ${materialKey}`)
                
                // 可选：BVH可视化
                if (this.mergeConfig.debugVisualization) {
                    const visualizer = new MeshBVHVisualizer(group.mesh, 10)
                    visualizer.update()
                    this.scene.add(visualizer)
                }
                
            } catch (error) {
                console.error(`[StaticGeometryMerger] BVH构建失败:`, error)
            }
        })
    }
    ...
    getPerformanceStats() {
        return {
            ...this.stats,
            drawCallReduction: `${this.stats.performanceGain}%`,
            objectReduction: this.stats.originalObjects > 0 
                ? Math.round((1 - this.stats.mergedObjects / this.stats.originalObjects) * 100) + '%'
                : '0%',
            vertexReduction: this.stats.originalVertices > 0 
                ? Math.round((1 - this.stats.mergedVertices / this.stats.originalVertices) * 100) + '%'
                : '0%',
            bvhGroups: this.bvhMap.size,
            mergeTime: `${Math.round(this.stats.mergeTime)}ms`
        }
    }
